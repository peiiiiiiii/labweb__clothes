...
import { addCart,LikeCommodity } from "./fornt__api.js";
...
const CART = {
    ...
    init(){
        ...
        let _wcontents = localStorage.getItem("wishlist");
        if(_wcontents){
            CART.wishlist = JSON.parse(_wcontents);
        }
        CART.count__cart();
    },
    async sync(){
    ...
    async addItemWish(img){
        ...
        CART.wishlist.push(obj);
        let _cart = JSON.stringify(CART.wishlist);
        localStorage.setItem("wishlist", _cart);
        let res = await LikeCommodity(PRODUCTS.CommodityId);
    },
    async singleSync(){
        // console.log(CART.contents);
        const obj = []
        let saveCart = CART.contents.map(item => {
            console.log(item)
            let { commodityID, color, size, amount } = item;
            obj.push({
    ...
    async count__cart(){
        ...
        if(count_s){
            ...
            document.querySelector(".js-item-count2").innerHTML = a;
        }else{
            document.querySelector(".js-item-count").innerHTML = 0;
            document.querySelector(".js-item-count2").innerHTML = 0;
        }
    },
    find(id,color,size){
        
        let match = CART.contents.filter(item=>{
            if((item.commodityID === id) && (item.color === color) && (item.size === size))
                return true;
        });
        if(match && match[0]){
            return match[0];
    ...
    reduce(id, qty=1, color,size){
        ...
        CART.contents.forEach(async item=>{
            if(item.commodityID === id && item.amount === 0 && (item.color === color) && (item.size === size))
    ...
    remove(id, color,size){
        CART.contents = CART.contents.filter(item=>{
            ...
            if(!(item.commodityID === parseInt(id) && (item.color === color) && (item.size === size)))
                return true;
        });
    ...
    totalPrice(){
        ...
        let a = CART.contents.map((item) => {
            totalCart += item.itemPrice * item.amount; 
        });
                ...
                <div class="product__list-item">
                    <figure>
                        ...
                        <figcaption>
                            ...
                            <div class="product__amount" data-id= ${item.commodityID}  data-quantity=${item.amount} data-color= ${item.color} data-size= ${item.size}>
                                <div class="product__amount-minus" data-id= ${item.commodityID}></div>
                                <div class="product__amount-curr">${item.amount}</div>
                                <div class="product__amount-plus" data-id= ${item.commodityID}></div>
                            </div>
                            </dl>
                        </figcaption>
                    </figure>
...
export function showWishlist(){
    // add id 把 ul 改掉
    // <div class="l-header__cart js-header-cartArea" id="cart__inner">
    let cartSection = document.getElementById('wish__inner');
    cartSection.innerHTML= "";
...
export async function showProducts( product,like_count ){
...
{/* 
    ...
    <div class="item__wrapper" >
        ...
        <div class="item__wrapper-left">
            ...
            <div class="u-margin-top-medium like">${like_count} 瀏覽次數</div>
        </div>
        ...
        <div class="item__wrapper-right">
            <div class="item__wrapper-list">
                ...
                <div class="wishlist__wrapper">
                    <button class="wishtlist" id="js-wishlistdetail">
                        <div>
...
export function addLikeItem(ev){
    
    ev.preventDefault();
    getslider0();
    CART.addItemWish(IMG);
    showWishlist();
}
